...
/* Orthographic: parallel projection, which is what an engineering view is.
 * Perspective makes two equal features different sizes depending on where they
 * sit, so a drawing cannot be read off it. */
...
/** Volume centroid of a closed triangle mesh, in millimetres.
 *
 * The divergence theorem over the tetrahedra formed by each triangle and the
 * origin. Averaging the vertices instead is the tempting version and is wrong:
 * it weights by tessellation density, so a cylinder with a finely meshed bore
 * pulls its centre toward the hole.
 *
 * Returns null if the signed volume is near zero, which means the mesh is not
 * closed and the answer would be arbitrary rather than merely imprecise.
 */
function centroidOf(positions, indices) {
  ...
  for (let i = 0; i < indices.length; i += 3) {
    ...
    const gx = positions[c], gy = positions[c + 1], gz = positions[c + 2];
    // Six times the signed volume of the tetrahedron (origin, a, b, c).
    const six = ax * (by * gz - bz * gy)
              - ay * (bx * gz - bz * gx)
              + az * (bx * gy - by * gx);
    volume += six;
    cx += (ax + bx + gx) * six;
...
function decodeMesh(mesh) {
  ...
  return {
    ...
    centroid: centroidOf(positions, indices),
  };
}
...
class Viewer {
  constructor(canvas) {
    ...
    //: Parallel projection. Off by default: perspective reads as a solid
  ...
  render() {
    ...
    // The orthographic frustum is sized from the camera distance so that
    // zooming keeps working and the part fills the same share of the frame in
    // both projections; switching should change the projection, not the shot.
    const projection = this.orthographic
      ? orthographic(this.camera.distance * Math.tan(Math.PI / 8), aspect, -100, 100)
    ...
    if (this.section.on) this._drawSectionPlane(modelView, projection, axis, where);
    if (this.showBounds) {
      this._drawBounds(modelView, projection);
      this._drawCentroid(modelView, projection);
    }
    if (this.measure.length) this._drawMeasure(modelView, projection);
  }

  /** The centre of mass, as a three-axis marker.
   *
   * Drawn where the part balances, which for a bracket is usually outside the
   * material. That is the useful thing about seeing it.
   */
  _drawCentroid(modelView, projection) {
    const gl = this.gl;
    const centre = this.mesh && this.mesh.centroid;
    if (!centre) return;
    const arm = 0.06 / this.scale;
    const line = [];
    for (let axis = 0; axis < 3; axis += 1) {
      const from = centre.slice(), to = centre.slice();
      from[axis] -= arm; to[axis] += arm;
      line.push(...from, ...to);
    }
    ...
    gl.bindBuffer(gl.ARRAY_BUFFER, this.buffers.centroid);
    ...
    gl.vertexAttrib3fv(this.lineAttributes.color, this.capColor);
    gl.disable(gl.DEPTH_TEST);
    gl.drawArrays(gl.LINES, 0, 6);
    gl.enable(gl.DEPTH_TEST);
  }

  /** The section plane's outline, so the cut has a position you can see.
   *
   * The slider says 42%. Of what, in which direction, is a question the
   * viewport should answer rather than the reader.
   */
  _drawSectionPlane(modelView, projection, axis, where) {
    const gl = this.gl;
    ...
    gl.disableVertexAttribArray(this.lineAttributes.color);
    gl.vertexAttrib3fv(this.lineAttributes.color, this.capColor);
    // In front of the part: the outline marks where the cut is, and half of it
    // is inside the material it cut.
    gl.disable(gl.DEPTH_TEST);
  ...
  _rayThrough(clientX, clientY) {
    ...
    // Under perspective every ray leaves the eye; under a parallel projection
    // they are all the view direction and it is the origin that slides across
    // the image plane. Picking has to follow the projection or the measure
    // tool reports distances between points nobody clicked.
    let origin = eye;
    let direction;
    if (this.orthographic) {
      const halfHeight = distance * Math.tan(Math.PI / 8);
      const offsetU = ndcX * halfHeight * aspect;
      const offsetV = ndcY * halfHeight;
      origin = [
        eye[0] + right[0] * offsetU + up[0] * offsetV,
        eye[1] + right[1] * offsetU + up[1] * offsetV,
        eye[2] + right[2] * offsetU + up[2] * offsetV,
      ];
      direction = forward;
    } else {
    ...
    return { origin: toModel(origin), direction };
  }
  ...
  project(point) {
    ...
    if (this.orthographic) {
      ...
      return {
        x: rect.width * 0.5 * (1 + x / (halfHeight * aspect)),
        y: rect.height * 0.5 * (1 - y / halfHeight),
      };
    }
    // Behind the eye has no place on screen; under perspective only.
